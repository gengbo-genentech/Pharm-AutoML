...
class FeaturePreprocessors():
    ...
    '''
    def __init__(self, X, y, result_path, grid_index, drop_features = [], result_logger=None, isDisplay = False):
        self.X = X
        self.y = y
        self.base_features = list(X.columns)
        self.result_path = result_path
        self.prepro_dir = self.result_path + '/' + str(grid_index)
    ...
    def feature_preprocessor_main(self, selection_params):
        """
        Args:
            selection_params: dictionary of parameters
        return:
            X_important: pandas dataframe, selected dataframe after feature reduction
            y_important: pandas series, target
        """
        removed_features, removed_rows = self.find_removableFeatures(selection_params)
    ...
    def find_removableFeatures(self, selection_params):
        ...
        if 'correlation_threshold' in selection_params.keys():
            ...
            if self.logger != None:
                self.logger.info(self.ops_cols['collinear'])
        else:
        ...
        print(self.ops_rows)
        if self.logger != None:
            self.logger.info('Data Preprocessor Summary')
            self.logger.info('%d total features out of %d identified for removal.' % (self.n_identified_cols, self.X.shape[1]))
            self.logger.info(self.all_identified_cols)
            self.logger.info(self.ops_cols)
            self.logger.info('%d total instances out of %d identified for removal.' % (self.n_identified_rows, self.X.shape[0]))
            self.logger.info(self.all_identified_rows)
            self.logger.info(self.ops_rows)
    ...
    def find_none_y(self, isRemove = True):
        ...
        if len(self.nan_y_index) != 0 and isRemove == True:
            ...
            if self.logger != None:
                self.logger.info("y with none value has been removed")
    ...
    def find_single_unique(self):
        ...
        self.ops_cols['drop_features'] = self.drop_features
        drop_info = 'manually drop %d features.' % len(self.ops_cols['drop_features'])
        print('Preprocessing Step 1: Manually Drop Features')
        print(drop_info)
        if self.logger != None:
            self.logger.info('Preprocessing Step 1: Manually Drop Features')
            self.logger.info(drop_info)
        ...
        if self.logger != None:
            self.logger.info('Preprocessing Step 2: Find Single Unique Features')
            self.logger.info(single_unique_info)
    ...
    def find_missing(self, missing_threshold, axis = 1):
        ...
        print(missing_info)
        if self.logger != None:
            self.logger.info('Preprocessing Step 3: Find Missing')
            self.logger.info(missing_info)

    def plot_missing(self, axis = 1):
        """Histogram of missing fraction in each feature"""
        if axis == 1:
            if self.record_missing_cols is None:
                if self.logger != None:
                    self.logger.error("Columns missing values have not been calculated. Run `find_missing with axis = 1`")
            ...
            if self.record_missing_rows is None:
                if self.logger != None:
                    self.logger.error("Rows missing values have not been calculated. Run `find_missing with axis = 0`")
    ...
    def find_collinear(self, correlation_threshold, one_hot=False):
        ...
        if self.logger != None:
            self.logger.info('Preprocessing Step 4: Find Collinear Features by Pearson Correlations')
            self.logger.info(collinear_info)
