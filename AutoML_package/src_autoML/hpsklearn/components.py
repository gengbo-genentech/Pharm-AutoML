...
def _lrc_C(name):
    return hp.loguniform(name, np.log(1e-5), np.log(1e1))

def _lrc_int_scaling(name):
    return hp.loguniform(name, np.log(1e-1), np.log(1e1))

def _lrc_l1_ratio(name):
...
#########################################################
##==== logistic regression classifier constructors ====##
#########################################################
def logistic_regression_classifier(name,
    ...
    def _name(msg):
        return '%s.%s_%s' % (name, 'lrc', msg)

    rval = scope.sklearn_lrc(
        penalty=_lrc_penalty(_name('penalty')) if penalty is None else penalty,
        # dual=_lrc_dual(_name('dual')),
        dual=dual,
        tol=_lrc_tol(_name('tol')),
        C=_lrc_C(_name('C')),
        fit_intercept=fit_intercept,
        intercept_scaling=_lrc_int_scaling(_name('intscaling')),
        ...
        n_jobs=n_jobs,
        l1_ratio=l1_ratio,
        # l1_ratio=(_lrc_l1_ratio(_name('l1ratio'))
...
def _xgboost_min_child_weight(name):
    return scope.int(hp.loguniform(name, np.log(1), np.log(10)))
...
def _xgboost_reg_lambda(name):
    return hp.loguniform(name, np.log(1), np.log(10))
...
def any_classifier(name):
    classifiers = [
        logistic_regression_classifier(name + '.lrc'),
        svc(name + '.svc'),
        knn(name + '.knn'),
