...
def _cost_fn(argd, X, y, EX_list, valid_size, n_folds, shuffle, random_state,
             use_partial_fit, info, timeout, _conn, n_repeats=None, loss_fn=None,
    ...
    try:
        ...
        if n_folds is not None:
            if n_folds == -1:
                ...
                if n_repeats is None:
                    ...
                    info('Will use repeated stratified K-fold CV with K:', n_folds,
                         'and n_repeats:', n_repeats)
                    try:
                        cv_iter = RepeatedStratifiedKFold(n_splits=n_folds,
                                                  n_repeats=n_repeats,
                                                  random_state=random_state
                                                 ).split(X, y)
                    except TypeError:
                        # Older syntax before sklearn version 0.18
                        cv_iter = RepeatedStratifiedKFold(y, n_folds=n_folds,
                                                  n_repeats=n_repeats,
                                                  random_state=random_state)
            else:
        ...
        for train_index, valid_index in cv_iter:
            ...
            if hasattr(learner, "partial_fit") and use_partial_fit:
                ...
                # print ('debug here  cost fn 4')
                # print ('learner')
                # print (type(learner))
                # print (learner)
                learner = copy.deepcopy(untrained_learner)
                learner.fit(XEXfit, yfit)
                # print ('debug here  cost fn 1')
                n_iters = None
            if learner is None:
                break
            # print ('debug here  cost fn 2')
...
class hyperopt_estimator(BaseEstimator):
    ...
    def fit_iter(self, X, y, EX_list=None, valid_size=.2, n_folds=None, 
                 ...
                 random_state=np.random.RandomState(), n_repeats=None,
        ...
        while True:
            ...
            if 'rstate' in inspect.getargspec(hyperopt.fmin).args:
                ...
                # print (self.algo)
                # print (self.trials)
                # print (len(self.trials.trials) + increment)
                # print (self.rstate)
                # print ()
                # print ()
